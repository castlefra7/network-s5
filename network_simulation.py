...
class Switch(Device):
    ...
    def listen_incoming_requests(self, packet):
        ...
        if type(packet) is ARPRequestPacket:
            to_txt("""{} envoie une requête de diffusion de type ARPRequest\n""".format(self.name))
            self.send_broadcast(packet)
        elif type(packet) is ARPResponsePacket:
            to_txt("{} envoie une réponse Unicast de type ARPResponse".format(self.name))
            self.send_unicast(packet)
        elif type(packet) is Frame:
...
class Router(Device):
    ...
    def __init__(self, _connected_switch, _routing_table, name, number_interfaces=10):
        super().__init__(name, None)
        self.arp_table = []
        if len(_connected_switch) > number_interfaces:
            raise Exception("{} a seulement {} interfaces".format(name, number_interfaces))
    ...
    def get_correct_ip_mac_mask(self, ip, mask):
        ...
        if correct_ip == "" or correct_mac == "":
            raise Exception("{} n'a pas cette interface réseau: {} - {}".format(self.name, net_addr, mask))
    ...
    def generate_arp_response(self, packet):
        desired_ip, desired_mac, desired_mask = self.get_correct_ip_mac_mask(packet.src_ip, packet.dest_mask)
        arp_response = ARPResponsePacket(src_ip=desired_ip, src_mac=desired_mac, dest_ip=packet.src_ip,
                                         dest_mac=packet.src_mac, mask=packet.dest_mask, src_orig_ip=packet.src_orig_ip)
        return arp_response
    ...
    def send_arp_response(self, packet):
        ...
        desired_switch.listen_incoming_requests(self.generate_arp_response(packet))
        ...
        if type(packet) is ARPRequestPacket:
            ...
            if packet.dest_mac != desired_mac:
                ...
                if packet.dest_ip != desired_ip:
                    ...
                    to_txt("{} reçoit un packet de type ARPResponse\n".format(self.name))
        elif type(packet) is Frame:
            to_txt("{} doit envoyer le packet vers {}".format(self.name, packet.dest_orig_ip))

            if self.is_local(packet):
                _new_packet = Packet(src_ip=packet.src_ip, src_mac=packet.src_mac, dest_ip=packet.dest_orig_ip,
                                     dest_orig_ip=packet.dest_orig_ip, data=packet.data, mask=packet.dest_mask,
    ...
    def send_to_next_hop(self, packet):
        ...
        if len(desired_gateways) <= 0:
            to_txt("--- Erreur:")
            to_txt("L'addresse de destination de ce packet ne se trouve pas dans le réseau")
        for desired_gateway in desired_gateways:
            desired_ip = ""
            desired_mask = ""
            found = False
            for route_info in desired_gateway.routing_table:
                network_analysis = NetworkAnalysis(route_info.ip, route_info.mask)
                net_addr = network_analysis.get_network_address()
            ...
            to_txt("{} doit envoyer le packet vers {}".format(self.name, desired_gateway.name))
            self.send_packet(packet)

    def is_local(self, packet):
        is_local = False
        network_analysis = NetworkAnalysis(packet.dest_orig_ip, packet.dest_mask)
        net_addr = network_analysis.get_network_address()
        for route_info in self.routing_table:
            if route_info.net_addr == net_addr:
                is_local = True
    ...
    def send_arp_request(self, packet):
        ...
        to_txt("{} envoie une requête ARP à {}\n".format(self.name, desired_switch.name))
        desired_switch.listen_incoming_requests(self.generate_arp_request(packet))

    def generate_arp_request(self, packet):
        desired_ip, desired_mac, desired_mask = self.get_correct_ip_mac_mask(packet.dest_ip, packet.dest_mask)
        arp_request = ARPRequestPacket(src_ip=desired_ip, src_mac=desired_mac, dest_ip=packet.dest_ip,
                                       mask=packet.dest_mask, dest_orig_ip=packet.dest_orig_ip,
                                       src_orig_ip=packet.src_orig_ip)
        return arp_request

    def send_frame(self, packet):
        desired_switch = self.get_correct_switch(packet.dest_ip)
        to_txt("{} envoie une Trame à {}\n".format(self.name, desired_switch.name))
    ...
    def generate_frame(self, packet):
        ...
        frame = Frame(src_ip=desired_ip, src_mac=desired_mac, dest_ip=packet.dest_ip, dest_mac=dest_mac,
                      data=packet.data, mask=packet.dest_mask, dest_orig_ip=packet.dest_orig_ip,
                      src_orig_ip=packet.src_orig_ip)
...
class Computer(Device):
    ...
    def send_frame(self, packet):
        to_txt("{} envoie une Trame à {}\n".format(self.name, self.switch.name))
        self.switch.listen_incoming_requests(self.generate_frame(packet))
        return

    def send_arp_request(self, packet):
        to_txt("{} envoie une reqête ARP à {}\n".format(self.name, self.switch.name))
        self.switch.listen_incoming_requests(self.generate_arp_request(packet))
    ...
    def generate_frame(self, packet):
        ...
        if dest_mac == "":
            raise Exception("Addresse MAC inconnue {}".format(packet.dest_ip))
        frame = Frame(src_ip=self.net_connect.net_info.ip, src_mac=self.net_connect.net_inter.mac,
                      dest_ip=packet.dest_ip, dest_mac=dest_mac, data=packet.data, mask=packet.dest_mask,
                      dest_orig_ip=packet.dest_orig_ip, src_orig_ip=packet.src_orig_ip)
        return frame

    def generate_arp_request(self, packet):
        arp_request = ARPRequestPacket(src_ip=self.net_connect.net_info.ip, src_mac=self.net_connect.net_inter.mac,
                                       dest_ip=packet.dest_ip, mask=packet.dest_mask,
                                       dest_orig_ip=packet.dest_orig_ip, src_orig_ip=packet.src_orig_ip)
        return arp_request

    def generate_arp_response(self, packet):
        arp_response = ARPResponsePacket(src_ip=self.net_connect.net_info.ip, src_mac=self.net_connect.net_inter.mac,
                                         dest_ip=packet.src_ip, dest_mac=packet.src_mac,
                                         mask=packet.dest_mask, dest_orig_ip=packet.dest_orig_ip,
                                         src_orig_ip=packet.src_orig_ip)
        return arp_response
    ...
    def is_local(self, packet):
        ...
        src_net_addr = NetworkAnalysis(self.net_connect.net_info.ip, self.net_connect.net_info.mask).get_network_address()
        if dest_net_addr != src_net_addr:
            return False
        return True

    def get_default_gateway(self):
        network_analysis = NetworkAnalysis(self.net_connect.net_info.ip, self.net_connect.net_info.mask)
        net_addr = network_analysis.get_network_address()
        desired_ip = ""
        desired_mask = ""
        for route_info in self.router.routing_table:
            if net_addr == route_info.net_addr:
                ...
                break
        if desired_ip == "":
            raise Exception("Le routeur n'a pas d'interface réseau pour l'addresse : {}".format(net_addr))
    ...
    def __str__(self):
        result = '-------------------------------------------------------\n'
        result += ("Informations de: {} - {} - {}\n".format(self.name, self.net_connect.net_info.ip,
                                                            self.net_connect.net_inter.mac))
        result += "Table ARP\n"
    ...
    def send_arp_response(self, packet):
        self.switch.listen_incoming_requests(self.generate_arp_response(packet))
    ...
    def listen_incoming_request(self, packet):
        ...
        if type(packet) is ARPRequestPacket:
            ...
            to_txt("###############################################")
            to_txt("Réception des données par: {} [{}]".format(self.name, self.net_connect.net_info.ip))
